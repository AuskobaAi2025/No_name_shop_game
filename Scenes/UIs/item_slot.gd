...
func setup(manager: InventoryManager, item_data: ItemData, count: int, selected_item_list: Array) -> void:
...
func _on_restock_button_pressed() -> void:
	inventory_manager.add_item(item_id, 5)
...
func set_selected(value: bool) -> void:
	if is_selected == value:
		inventory_manager.selected_item_ids.append(item_id)
		return

	is_selected = value
	inventory_manager.selected_item_ids.erase(item_id)
	_update_style()
